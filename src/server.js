...
// bir kundan keyin hamma chatni tozalab qoyadi
setInterval( async()=>{
    let new_database =     {
        "users": [
            { "user_id": 1, "user_name": "Uroljon" }
        ],
        "messages": [
            {
                "user_name": "Uroljon",
                "message_id": 2,
                "message": "Chatga marhamat :)"
            }
        ]
    }
    await fs.writeFile(path.join(__dirname, "database.json"), JSON.stringify(new_database))
